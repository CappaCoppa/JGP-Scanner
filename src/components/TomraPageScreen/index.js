...
const TomraPageScreen = ({ navigation }) => {
  const { sessionObject } = useContext(FirstLevelContext);

  const tomra = sessionObject.tomra;
  ...
  const sectionListItem = ({ time }, section, index) => {
    const sectionIndex = tomra.qr_scan.indexOf(section);
    return (
      <View
        style={{
          margin: 10,
      ...
      >
        <Pressable
          disabled={
            tomra.qr_scan[sectionIndex].data[index].is_completed ? true : false
          }
          onPress={() => {
          ...
          style={
            tomra.qr_scan[sectionIndex].data[index].is_completed
...
export default TomraPageScreen;
